...
//Create a simple window
	...
	SDL_Window* Window = SDL_CreateWindow("Clash of clan", center, center, width, height, SDL_WINDOW_OPENGL);
	...
	float vertices[] = {
		-0.3f, 0, 0.0f,  0,0,ColorValue,
		 0, 0.3f, 0.0f,	0,0,ColorValue,
		 0,  0, 0.0f,	ColorValue,0,ColorValue,
		 ...
		 0, 0, 0.0f,       ColorValue,0,ColorValue,
		 0,  0.3f , 0,     0,0,ColorValue,
	     0.3f, 0.3f, 0.0f, 0,0,ColorValue,
	...
	while (isRunning) {
		...
		float timeValue = (float)SDL_GetTicks() / RandomValue;
		float redColor = (sin(timeValue) / 2.0f) + 0.5f;
		int vertexColorLocation = glGetUniformLocation(shaderProgram, "uniColor");
		glUseProgram(shaderProgram);
		glUniform4f(vertexColorLocation, redColor, 0.0f, 1.0f, 0.0f);


		if (Condition)
		{
			SecondColor =+SecondColor + timeValue / RandomY;
			ThirdColor = ThirdColor + timeValue / RandomX;
		}
		else
		{
			SecondColor = SecondColor - timeValue / RandomY;
...
;
		...
		int vertexColorLocationMovementy = glGetUniformLocation(shaderProgram, "yAnim");
		glUseProgram(shaderProgram);
		glUniform1f(vertexColorLocationMovementy, SecondColor);


		int vertexColorLocationMovementx = glGetUniformLocation(shaderProgram, "xAnim");
		glUseProgram(shaderProgram);
		glUniform1f(vertexColorLocationMovementx, ThirdColor);

		if (SecondColor > 1.7f or ThirdColor > 1.7f)
		{
			timeValue = 0;
			Condition = false;
			ColorValue = 10;
			RandomValue = (rand() % (5000 - 1000)) + 1000;

			RandomY = (rand() % (200 - 10)) + 10;
			RandomX = (rand() % (200 - 10)) + 10;

		}
		if (SecondColor < 0.3 or ThirdColor < 0.3)
		{	
			timeValue = 0;
			Condition = true;
			ColorValue = 0;
			RandomValue = (rand() % (1000 - 100)) + 100;
